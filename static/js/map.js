...
var simulation_manager = (function(){
    ...
    var ua_is_mobile = navigator.userAgent.indexOf('iPhone') !== -1 || navigator.userAgent.indexOf('Android') !== -1;	
		...
		var show_buildings = 0
	
		$('#showbuildings').change(function(){
		if (show_buildings > 0)	{
			show_buildings = 0;} 
		else {
			show_buildings = 1;}
        ...
        function init(){
            ...
            var map_options = {
                ...
                mapTypeControlOptions: {
                    ...
                    mapTypeIds: ['stamen_toner','stamen_lite']
                }
            };
			...
			//new
			map.data.loadGeoJson('/api/geojson/ptv/train_route_shapes_ptv.geojson');
			var featureStyle = {
    		fillColor: 'green',
    		strokeWeight: 3	
  			}
  			map.data.setStyle(featureStyle);            
            var stamen_toner_lite = new google.maps.StamenMapType('toner-lite');
            stamen_toner_lite.set('name', 'Toner Lite');
            map.mapTypes.set('stamen_lite', stamen_toner_lite);
			
            var stamen_toner = new google.maps.StamenMapType('toner');
            stamen_toner.set('name', 'Toner');
            map.mapTypes.set('stamen_toner', stamen_toner);
            ...
            function map_layers_add(){
                ...
                ft_id = config.getParam('ft_layer_ids.vampire2011');
                if (show_vampire> 0) {
                    edges_layer = new google.maps.FusionTablesLayer({
                        query: {
                            select: 'geometry',
				...
				var a = config.getParam('test_stops');
				if (show_stops > 0)	{
					for (i=0; i< a.length; i++) {
						var color_config = ['#000000'];
						var radius = a[i][2]*4;	//Math.sqrt(a[i][2]);
						if (a[i][3] < 3){
							//train
							color_config = ["#BC00FF", 0.4];}
						else{
							//tram
							color_config = ['#2600FF', 0.2];}
								
						var test = {
						strokeColor: '#000000',
						strokeOpacity: 0.1,
						strokeWeight: 2,
						fillOpacity: color_config[1],
						map: map,
						center: new google.maps.LatLng(a[i][0], a[i][1]),
						radius: radius,

						fillColor : color_config[0]
						};
						circle = new google.maps.Circle(test);
					}	
				}
// ---------------------------------------------------------------------------				
				var b = config.getParam('building_approvals');
				if (show_buildings > 0)	{
					for (i=0; i< b.length; i++) {
						var coords = [
							new google.maps.LatLng(b[i][0], b[i][1]),
							new google.maps.LatLng(b[i][2], b[i][3]),
							new google.maps.LatLng(b[i][4], b[i][5]),
							new google.maps.LatLng(b[i][0], b[i][1])
						];
						dwelling = new google.maps.Polygon({
							paths: coords,
							strokeColor: '#000000',
							strokeOpacity: 0.1,
							strokeWeight: 2,
